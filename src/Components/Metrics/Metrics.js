...
class Metrics extends Component {
	...
	everyLetter = () => {
		...
		if (
			this.props.userInputAscii.length === this.props.snippetAscii.length &&
			this.props.snippetAscii.length !== 0
		) {
			this.endOfSnippet();
			this.props.toggleReadOnly();
		} else if (
			this.props.userInputAscii[this.props.userInputAscii.length - 1] !==
			this.props.snippetAscii[this.props.userInputAscii.length - 1]
		) {
			this.allErrors++;
		}
		this.WPM();
